...
;(function ($, window, document, undefined) {
  ...
  function Owl(element, options) {
    ...
    /**
     * Coordinates of all items in pixels.
...
/**
 * Prepares an item before adding.
 ...
 * @todo Horizontal swipe threshold as an option
 * @todo #261
 * @protected
...
/**
 * Gets the absolute position of the closest item for a coordinate.
...
Owl.prototype.closest = function (coordinate, direction) {
  ...
  if (!this.settings.freeDrag) {
    // Check the closest item
    $.each(coordinates, $.proxy(function (index, value) {
      // On a left pull, check on the current index
      if (direction === 'left' && coordinate > value - pull && coordinate < value + pull) {
        position = index;
      }
      // On a right pull, check on the previous index
      // To do so, subtract width from value and set position = index + 1
      else if (direction === 'right' && coordinate > value - width - pull && coordinate < value - width + pull) {
        position = index + 1;
      }
      // Check if the coordinate is between the current and next item
 ...
 * @protected
 * @param {Number|String|Object|Array|Boolean|RegExp|Function|Symbol} input - The input to be tested
 * @returns {Boolean} - An indication if the input is a Number or can be coerced to a Number
 */
Owl.prototype.isNumeric = function (input) {
 ...
 * @param {Number} [speed] - The time in milliseconds for the transition.
 * @param {Boolean} [standard=false] - Whether to use the standard behavior or not.
 */
